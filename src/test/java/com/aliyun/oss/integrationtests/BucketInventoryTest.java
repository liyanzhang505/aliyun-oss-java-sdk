...
public class BucketInventoryTest extends TestBase {
    ...
    private InventoryConfiguration createTestInventoryConfiguration(String configurationId) {
        ...
        InventoryDestination destination = new InventoryDestination().withS3BucketDestination(ossBucketDestin);
    ...
    public void testBucketInventoryNormal() {
        ...
        try {
            ...
            Assert.assertEquals(InventoryIncludedObjectVersions.Current.toString(), actualConfig.getIncludedObjectVersions());
            Assert.assertEquals("testPrefix", actualConfig.getInventoryFilter().getPrefix());
            Assert.assertEquals(InventoryFrequency.Daily.toString(), actualConfig.getSchedule().getFrequency());
            ...
            Assert.assertEquals("123", actualDestin.getEncryption().getServerSideKmsEncryption().getKeyId());
        } catch (ClientException e) {
            e.printStackTrace();
            Assert.fail(e.getMessage());
        } finally {
            ossClient.deleteBucketInventoryConfiguration(new DeleteBucketInventoryConfigurationRequest()
                    .withBucketName(bucketName).withInventoryId(inventoryId));
        }
    }
